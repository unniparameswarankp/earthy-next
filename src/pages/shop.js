...
export async function getServerSideProps(context) {
  ...
  const productsQuery = `*[_type == "product"] | order(_createdAt desc) [${start}...${start + PRODUCTS_PER_PAGE}]{
    _id,
    title,
    slug,
    price,
...
const Shop = ({ products, categories, totalPages, currentPage }) => {
  ...
  {prod.price != null
    ...
    : 'Price is not available'}
                    ...
                    <Link href={`/product/${prod.slug.current}`} className="btn-1 green">View Product</Link>
                  </div>
                </div>
              </div>
            </div>
